  ...
  override def fix(implicit doc: SemanticDocument): Patch = {
    ...
    object SemanticParserType {
      def unapply(tpe: SemanticType): Boolean = tpe match {
        case TypeRef(_, symbol, _) => symbol.toString == "cats/parse/Parser1#" || symbol.toString == "cats/parse/Parser#"
        case _ => false
      }
    }
    ...
    doc.tree.collect {
      ...
      case sel @ Term.Select(from, t @ Term.Name(nme)) => {
        if (from.symbol.toString() == "cats/parse/Parser.")
          parserModuleRenames.get(nme).map(newName => Patch.replaceTree(sel, s"$pModuleName.$newName")).getOrElse(Patch.empty)
        else
          from.symbol.info.map(_.signature).collect {
            case MethodSignature(_, _, SemanticParserType()) => pClassRenames.get(nme).map(newName => Patch.replaceTree(t,newName))
            case x if nme.endsWith("1") => {
              println("what is $x")
              None
            }
